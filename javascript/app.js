...
/* --------------- functionality for clicking li's images ---------------------- */
	...
	// on hover, the state's name appears
...
/* --------------- functionality for '#submit-answer' button ----------------- */
		...
		// NEED TO PUSH ANSWER TO RESULTS ARRAY
			// $('#submit-answer').on('click', function(){
			// 	// what to do if attempts === maxAttempts
				...
				var flagName = $('#flag > img').attr('alt');
					...
					if(questionNumber <= gameLength){
							if(playerChoice === flagName){
								...
								$('#correct-answers ul').append("<li>"+flagName+"</li>");
								console.log("correctResults = " + correctResults);
							} else if(playerChoice !== flagName && attempts === maxAttempts){
								// incorrect
								incorrectResults.push(flagName);
								$('#incorrect-answers ul').append("<li>"+flagName+"</li>");
